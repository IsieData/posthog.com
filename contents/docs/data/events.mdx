...
#### Server-side SDK properties

Server-side SDKs like [Node.js](/docs/libraries/node) and [Python](/docs/libraries/python) add properties to distinguish their events from browser and mobile clients:

| Name            | Key          | Value  |
|-----------------|--------------|--------|
| Server          | `$is_server` | `true` |

They also include `$lib` (e.g., `posthog-node`, `posthog-python`) and `$lib_version` to identify which SDK captured the event.
