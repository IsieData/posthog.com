...
For static cohorts, you can ask the agent to populate them from a SQL query or insight actors. This is the recommended approach for larger sets of users since there's no row limit. For small ad-hoc additions, you can also add or remove persons by their UUIDs.

Example prompts:

- "Create a dynamic cohort called 'power-users' for users who completed the purchase event more than 5 times"
- "Create a static cohort of everyone who did a pageview in the last 7 days"
- "Save the people behind my funnel drop-off as a static cohort called 'checkout-abandoners'"
