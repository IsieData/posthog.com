...
When the bot is first added to a channel, it automatically posts a welcome message with example `@PostHog` prompts and a link to the docs. The welcome only appears once per channel – removing and re-adding the bot won't trigger a second post.
