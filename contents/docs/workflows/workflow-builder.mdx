...
You define the audience using person properties, cohorts, or metadata. Feature flags can't be used as a batch audience filter because flag evaluation is dynamic (rollout percentages, holdouts, and `distinct_id` hashing), so it can't produce a static, queryable audience.
...
- **Audience filters** — who should enter the workflow (e.g. person properties, cohorts, metadata)
