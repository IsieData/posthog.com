...
When you select tables to sync, PostHog checks whether row-level security (RLS) is active for the sync role on each table. If RLS policies apply to the connecting user, a warning tag appears next to the table name in the table picker:

**"RLS may hide rows"** – Row-level security is active on this table for the sync role, so PostHog can only read rows the policy permits and can't detect how many rows are hidden.

This warning is advisory. You can still sync the table, but keep in mind:

- PostHog syncs only the rows visible to the sync role under the active RLS policies.
- You won't know how many rows the policy excludes.
- The sync shows the row count visible to PostHog, not the total row count in the table.

### Resolving the warning

To sync all rows regardless of RLS policies, either:

- **Grant `BYPASSRLS`** to the sync role so it bypasses row-level security entirely:

  ```sql
  ALTER USER "posthog" WITH BYPASSRLS;
  ```

- **Create a permissive policy** that allows the sync role to read all rows:

  ```sql
  CREATE POLICY allow_posthog_full_read ON "your_table"
    FOR SELECT
    TO "posthog"
    USING (true);
  ```

If the RLS policy is intentional and you only want PostHog to see a subset of rows (for example, to exclude sensitive data), ignore the warning. The sync works correctly with the rows the policy permits.

### Sync-time RLS detection

For existing sources, PostHog also checks for RLS during each sync. If row-level security becomes active on a table after the source was created (or if you missed the warning during setup), you'll see a warning in the sync logs:

> Row-level security is active for the sync role on schema.table_name (rows visible to this sync: X). Grant the role BYPASSRLS or a permissive policy if it should see all rows.

Check the **Syncs** tab on your source to view logs and identify affected tables.
...
If your sync completes successfully but returns zero rows (or fewer than expected), check the sync logs for a row-level security warning. RLS policies can silently filter out all rows if the sync role doesn't match the policy criteria. See [Row-level security warning](#row-level-security-warning) for how to resolve this.
