...
Row filters let you sync only rows that match specific conditions. For example, you can filter to sync only rows where `created_at > '2026-01-01'` or `status = 'active'`.
...
Row filters support integer, numeric, string, boolean, date (`YYYY-MM-DD`), and timestamp (`YYYY-MM-DDTHH:MM:SS`) columns. Filters are applied on the next sync and don't remove rows that were already synced.

See the [row filters documentation](/docs/cdp/sources#row-filters) for supported operators and value formats.
