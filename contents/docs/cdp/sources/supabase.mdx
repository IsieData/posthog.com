...
<CalloutBox icon="IconWarning" title="Row-level security may hide data" type="caution">

Supabase enables row-level security (RLS) on many tables by default. If RLS is active for your sync role, PostHog can only read rows the policy permits—syncs may return zero rows or fewer than expected. PostHog shows an **"RLS may hide rows"** warning in the table picker and logs a warning during sync if this applies.

To sync all rows, either grant `BYPASSRLS` to the sync role or create a permissive policy. See the [Postgres RLS documentation](/docs/cdp/sources/postgres#row-level-security-warning) for details.
