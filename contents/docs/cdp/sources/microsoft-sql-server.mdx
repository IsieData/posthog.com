...
3. Enter your database connection details:
   ...
   - **Schema:** (Optional) The schema where your tables are located. Leave blank to import tables from all schemas, or specify a single schema like `dbo` to import only from that schema.
4. Click **Link**
...
Leave the **Schema** field blank to import tables from all non-system schemas. Tables appear with qualified names like `schema.table` (for example, `sales.orders` or `analytics.users`), so tables with the same name in different schemas don't conflict.

System schemas like `sys`, `guest`, and `INFORMATION_SCHEMA` are automatically excluded.

To import from only one schema, specify it in the **Schema** field.
