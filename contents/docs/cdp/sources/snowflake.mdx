...
- **Schema:** (Optional) The schema where your tables are located. Leave blank to import tables from all schemas, or specify a single schema like `RAW_POS` to import only from that schema. If it isn't working, try using all caps.
...
Leave the **Schema** field blank to import tables from all non-system schemas in your database. Tables appear with qualified names like `schema.table` (for example, `RAW_POS.menu` or `ANALYTICS.customers`), so tables with the same name in different schemas don't conflict.

PostHog automatically excludes system schemas like `INFORMATION_SCHEMA`.

To import from a single schema, enter it in the **Schema** field.
