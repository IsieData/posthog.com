...
5. Fill in the necessary [configuration details](#configuration).
...
* **Access Key ID (required):** An access key ID with access to the S3 bucket.
* **Secret Access Key (required):** A secret access key with access to the S3 bucket.
...
* **Endpoint URL (required):** The endpoint URL for your provider (e.g., `https://<account-id>.r2.cloudflarestorage.com` for Cloudflare R2). Must resolve to a publicly accessible address.
