...
## S3 destination types

PostHog provides two S3 destination types:

- **AWS S3** - Use for AWS-native S3 buckets. Supports server-side encryption (`AES256` or `aws:kms`) and KMS key IDs. Does not support custom endpoint URLs.
- **S3-compatible** - Use for S3-compatible storage providers like MinIO, Cloudflare R2, or Google Cloud Storage. Requires an endpoint URL. Does not support AWS-native encryption options.

When creating a new batch export, select either **AWS S3** or **S3-compatible** as your destination type.

> **Note:** The legacy `S3` destination type is deprecated and can no longer be used to create new batch exports. Existing `S3` batch exports continue to work and can be edited. For new exports, use **AWS S3** or **S3-compatible** instead.
...
3. Search for **S3** and select either **AWS S3** (for AWS S3) or **S3-compatible** (for S3-compatible storage providers).
4. Click the **+ Create** button.
5. Fill in the necessary [configuration details](#s3-configuration).
6. Finalize the creation by clicking **Create**.
7. Done! The batch export will schedule its first run on the start of the next period.

## S3 configuration

Configuration options vary depending on which destination type you selected.

### AWS S3 configuration

The **AWS S3** destination type supports the following configuration values:
...
The **S3-compatible** destination type supports the following configuration values:
...
- **Max file size (MiB):** If the size of the exported data exceeds this value, the data is split into multiple files. (Note that this is approximate and the actual file size may be slightly larger). If this value is not set, or is set to 0, the data is exported as a single file.
- **Compression:** Select a compression method (like gzip) to use for exported files or no compression.
- **AWS Access Key ID (required):** An access key ID with access to the S3-compatible bucket.
- **AWS Secret Access Key (required):** A secret access key with access to the S3-compatible bucket.
- **Endpoint URL (required):** The endpoint URL of your S3-compatible storage provider. Must resolve to a publicly accessible address (internal or private IPs are not allowed). See [S3-compatible blob storage](#s3-compatible-blob-storage) for provider-specific examples.
- **Events to exclude:** A list of events to omit from the exported data.
...
> **Note:** When creating a new batch export for an S3-compatible provider, select the **S3-compatible** destination type.

PostHog S3 batch exports can export data to S3-compatible blob storage like [MinIO](https://github.com/minio/minio), [Cloudflare R2](https://www.cloudflare.com/developer-platform/products/r2/), or [Google Cloud Storage (GCS)](https://cloud.google.com/storage). Here we describe configuration tweaks required for S3-compatible storage destinations that we have tested.
