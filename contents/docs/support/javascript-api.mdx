...
interface SendMessageResponse {
  ...
  ticket_number?: number  // Human-readable ticket number (e.g. 1234)
...
// Get the human-readable ticket number (null if no conversation started)
...
The **ticket number** is a sequential number (like 1234) that matches what you see in the PostHog support UI. Unlike the ticket ID (a UUID), it's easier for users to reference.
...
| `getCurrentTicketNumber()` | Get current ticket number | `number \| null` |
