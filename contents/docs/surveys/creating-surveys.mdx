...
-   **User sends events:** Display a survey to users who send a specific event. The survey shows on the page load where the event is captured, not on a later load or reload. To target users based on past behavior, use person, group, or cohort properties instead.
