...
> **Warning:** If you use "any" matching with **100% sampling** and other trigger conditions (URL triggers, event triggers, or a feature flag), every session gets recorded. The 100% sampling condition always matches, so your other conditions have no effect. To use trigger conditions as filters, either lower the sampling rate below 100% or switch to "all" matching.
