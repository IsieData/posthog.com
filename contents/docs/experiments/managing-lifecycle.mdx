...
When you archive an experiment, PostHog also archives its linked feature flag automatically when:

- The flag is already disabled (not serving traffic)
- No other live experiment uses the same flag

If the flag is still enabled (for example, you're rolling out the winning variant), it won't be auto-archived. This avoids disrupting your rollout. You can [archive the flag manually](/docs/feature-flags/creating-feature-flags#archiving-feature-flags) after you've hardcoded the winning variant and removed the flag check from your code.

When you unarchive an experiment, its auto-archived flag is also unarchived (but remains disabled). Re-enabling the flag is a separate action. Flags you archived manually are not affected by unarchiving the experiment.
