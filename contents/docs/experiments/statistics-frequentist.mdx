...
Standard p-values and confidence intervals assume you check results once at a fixed sample size. If you peek at results repeatedly, each check inflates the false positive rate.

[Sequential testing](/docs/experiments/sequential-testing) solves this by producing always-valid p-values that stay valid under continuous monitoring. The tradeoff is wider confidence intervals. Enable it when you plan to check results frequently before deciding to ship or stop.
