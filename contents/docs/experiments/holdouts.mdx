...
## How holdouts appear in experiments

When you assign a holdout to an experiment, it appears in the Distribution table on the **Variants** tab so you can see how many users are in the holdout. Holdouts are excluded from metric analysis.

Why? Comparing holdout users to control wouldn't measure your experiment's effect. Holdout users never saw the experiment, so any metric difference reflects cumulative product impact over time – not the specific change you're testing.

To measure holdout impact, compare the holdout group's metrics to your overall user base in [Product analytics](/docs/product-analytics) or create a dedicated analysis using [Cohorts](/docs/data/cohorts).
