...
## Configuring the baseline variant

By default, the `control` variant is the baseline for all statistical comparisons. All other variants are compared against it to calculate deltas, significance, and win probability.

You can change the baseline to any variant in your experiment. To do this, go to the experiment's **Settings** tab and select a different variant from the **Baseline variant** dropdown.

Changing the baseline takes effect immediately for all result calculations. This is useful when:

- Your experiment doesn't have a variant named `control`
- You want to compare all variants against a specific treatment variant
- You're using a different baseline convention
