...
To remove person properties, use `unsetPersonProperties` with the property name (or an array of names) you want to remove. This creates a `$set` event with a `$unset` array, which removes those properties from the person's profile.
