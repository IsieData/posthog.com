...
To remove person properties, call `unsetPersonProperties` with the property name, or an array of names, you want to remove. This captures a `$set` event with a `$unset` array, which removes those properties from the person's profile.
