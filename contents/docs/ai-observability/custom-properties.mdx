...
If you're using [LiteLLM](/docs/ai-observability/installation/litellm), you can also pass custom properties through its native `metadata` parameter. PostHog extracts these automatically and adds them to event properties, so standard LiteLLM workflows work without a PostHog-specific wrapper.
...
When using [LiteLLM](/docs/ai-observability/installation/litellm), pass custom properties through the standard `metadata` parameter:
...
PostHog extracts these keys and adds them to the `$ai_generation` event properties. Internal properties (prefixed with `$ai_`) and `user_id` are protected and can't be overwritten through metadata.
