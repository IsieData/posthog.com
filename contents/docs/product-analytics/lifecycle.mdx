...
Click **Options** to enable display options:

- **Show values on series** – displays the count for each segment on the chart.
- **Show percentages on series** – shows each segment's share of total activity for that time interval. When combined with values, labels display as `580 (42%)`.
