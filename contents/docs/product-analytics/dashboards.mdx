...
Click the dropdown on the **Add...** button and select **Recent events** under the **Activity** group to add a live events feed to your dashboard. This widget displays recent events captured in your project, mirroring the [Activity > Explore](https://app.posthog.com/activity/explore) page.

Each row shows the event name, person, timestamp, library, and URL. Click any row to open the event's detail page in a new tab. The tile title links to **Activity > Explore** for the full events view.

You can configure the widget with:

- **Date range** – Filter events by time period (default: last 24 hours).
- **Limit** – Number of events to display (default: 25, max 50).
