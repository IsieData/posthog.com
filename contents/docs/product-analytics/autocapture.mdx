...
| `css_selector_ignorelist`<br/>**Type:** `string[]`                 | List of CSS selectors to disable autocapture on (e.g., `['[data-ph-no-autocapture]']`). We consider the element tree from root to target, so a click is ignored if the target or any of its parents matches. Defaults to `['.ph-no-autocapture', '[data-ph-no-autocapture]']`. Providing a custom list replaces the defaults, and an empty list (`[]`) disables the default opt-out selectors. See [opting elements out of autocapture](#opting-elements-out-of-autocapture). |
...
#### Opting elements out of autocapture

To stop interaction autocapture on a specific element, add the `ph-no-autocapture` class or the `data-ph-no-autocapture` attribute. PostHog checks the clicked element and its parents, so any interaction on a matching element or its children is ignored.

```html
<button class="ph-no-autocapture">This click won't be autocaptured</button>
<div data-ph-no-autocapture>Clicks inside here won't be autocaptured</div>
```

Unlike the `ph-no-capture` class, these selectors only affect interaction autocapture. They don't hide the element in [session replay](/docs/session-replay), so use them when you want to opt out of autocapture but still see the element in recordings. To exclude an element from both autocapture and session replay, use `ph-no-capture` instead.

To customize which selectors are ignored, use the `css_selector_ignorelist` option. A custom list replaces the defaults, so include `.ph-no-autocapture` and `[data-ph-no-autocapture]` if you still want them respected. An empty list (`[]`) disables the default opt-out selectors entirely.
