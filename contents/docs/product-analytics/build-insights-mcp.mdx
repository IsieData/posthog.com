...
- `What dashboard templates are available? Create a dashboard using the website template.`
...
| Tool | Description |
|------|-------------|
| `dashboard-create` | Create a new dashboard. Optionally pass `use_template` with a template key to start from a predefined template. |
...
| `dashboard-templates-list` | List available dashboard templates. Returns each template's key (`template_name`), description, tags, and scope. |
| `dashboard-templates-retrieve` | Get a single template's tile summary to preview what it builds before creating a dashboard from it. |
