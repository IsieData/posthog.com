...
PostHog sends a weekly digest email summarizing your Error Tracking activity. It includes total exception counts, top issues, new issues, and crash-free session rates.

When a significant percentage of your stack traces are unreadable due to missing source maps, the digest includes a recommendation to upload them. It shows the percentage of unreadable stack traces and provides the wizard command to run from your project root:

```bash
npx -y @posthog/wizard@latest upload-source-maps
```

For EU Cloud users, the command includes `--region eu`:

```bash
npx -y @posthog/wizard@latest upload-source-maps --region eu
```

This recommendation only appears when source maps haven't been uploaded and would meaningfully improve stack trace readability. See the [source maps docs](/docs/error-tracking/upload-source-maps) for more details on uploading source maps.
