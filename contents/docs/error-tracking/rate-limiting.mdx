...
If you're using PostHog's [MCP server](/docs/model-context-protocol), you can read and update rate limit settings using the `error-tracking-settings-get` and `error-tracking-settings-update` tools. This lets AI agents manage rate limits without visiting the PostHog UI.
