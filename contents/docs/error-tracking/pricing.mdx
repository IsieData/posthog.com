...
Server-side rate limits cap exception ingestion at the project level. Unlike client-side burst protection which runs in the SDK, these limits are enforced by PostHog and apply to all exceptions from every client.

You can configure two types of rate limits in your [Error Tracking settings](https://app.posthog.com/error_tracking/configuration#selectedSetting=error-tracking-rate-limits):

- **Project-wide rate limit** – Caps the total number of exceptions ingested across your entire project.
- **Per-issue rate limit** – Caps exceptions per individual issue, preventing a single noisy error from consuming your quota.

When a rate limit is reached, additional exceptions are dropped and not billed. PostHog shows a recommendation in the **Recommendations** tab when rate limits aren't configured.
