...
You can filter flags by tag in the flag list. There's also an **Exclude tags** filter that hides flags carrying specific tags — useful for removing noise from teams or projects you don't work on.
...
Archiving is for flags you're done with but want to keep for historical reference — for example, a flag tied to a finished experiment or a completed rollout. Unlike deleting, archiving preserves all linked data (experiments, surveys, etc.) while removing the flag from your day-to-day list.

Key rules for archived flags:

- **Archived flags must be disabled.** You can archive an enabled flag, but PostHog will disable it as part of the archive action. A confirmation dialog warns you that this will immediately stop serving the flag to users.
- **Archived flags are hidden by default.** They don't appear in the flag list, bulk operations, or "select all matching" results unless you filter for them.
- **Archived flags can't be re-enabled directly.** Unarchive the flag first, then enable it.

To archive a flag, open the flag's detail page or the row menu in the flag list and select **Archive feature flag**. To find archived flags, use the **Status** filter and select **Archived**. You can unarchive from the same menu or from the banner shown on an archived flag's detail page.

#### Archive vs. disable vs. delete

| Action      | Flag serves traffic?  | Visible in list?  | Linked data kept? | Reversible?                  |
| ----------- | --------------------- | ----------------- | ----------------- | ---------------------------- |
| **Disable** | No                    | Yes               | Yes               | Yes — re-enable anytime      |
| **Archive** | No (must be disabled) | Hidden by default | Yes               | Yes — unarchive anytime      |
| **Delete**  | No                    | No (soft delete)  | Yes               | Yes — restore from flag page |
...
Some flags can't be deleted until you remove whatever references them. When deletion is blocked, PostHog opens a dialog listing exactly which resources are in the way, each with a direct link so you can resolve the blocker without searching. Deletion is blocked when the flag is:
