...
<CalloutBox icon="IconWarning" title="Local evaluation limitation" type="caution">

Early exit only takes effect when flags are evaluated through the `/flags` API endpoint. Server-side SDKs using [local evaluation](/docs/feature-flags/local-evaluation) evaluate all condition sets regardless of this setting.
