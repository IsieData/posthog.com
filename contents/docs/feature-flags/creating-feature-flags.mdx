...
By default, PostHog evaluates all condition sets and returns `true` if any condition passes. Enable **Stop evaluation at first matching condition set** to stop evaluation at the first condition where property filters match – even if the user falls outside the rollout percentage.

This is useful when you want users who match a condition's targeting criteria but miss the rollout to get a consistent `false` instead of potentially matching a later, broader condition.

**Example:** You have a flag with two conditions:
1. Users in the "Beta testers" cohort at 50% rollout
2. All users at 10% rollout

Without early exit, a beta tester who falls outside the 50% rollout can still match the 10% rollout in condition 2. With early exit enabled, that beta tester gets `false` immediately after failing the rollout check in condition 1.

<CalloutBox icon="IconInfo" title="Local evaluation support" type="fyi">

Early exit is supported in both the `/flags` API endpoint and [local evaluation](/docs/feature-flags/local-evaluation) for the Node SDK. Other server-side SDKs using local evaluation evaluate all condition sets regardless of this setting.
