...
### Bulk scanning multiple recordings

To run a scanner across several recordings at once:

1. Go to **Session replay** and select the recordings you want to scan.
2. Open the bulk actions menu (the three-dot menu).
3. Hover over **Scan these recordings** to see available scanners.
4. Select a scanner to start scanning all selected recordings.

PostHog shows a toast notification with the number of scans started and a link to the scanner's page. The backend automatically skips duplicate (scanner, session) pairs, so re-running the same scanner on recordings it's already observed is safe.
