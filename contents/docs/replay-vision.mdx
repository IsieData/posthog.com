...
| --- | --- | --- |
| **Monitor** | A verdict (`yes`, `no`, or optionally `inconclusive`), with reasoning | Flagging whether a specific condition occurred, e.g. "did someone get stuck with no clear next step?" |
...
3. Write a prompt describing what the scanner should look for. Be concrete and avoid speculation – e.g. *"Answer 'yes' if the user appears stuck on a page – scrolling without engaging, hovering over elements with no clear CTA, or abandoning shortly after arriving."*
4. Configure the type-specific options:
   - **Monitor** – optionally enable **Allow inconclusive** to let the model return `inconclusive` when the session doesn't provide enough signal to decide; otherwise verdicts are constrained to `yes` or `no`.
...
| `scanner_output_verdict` | (Monitor) the verdict: `yes`, `no`, or `inconclusive` |
...
For example, to count sessions flagged `yes` by a "Dead-end pages" monitor scanner over the last week:
...
When you enable **Signals** on a scanner, notable findings (such as a monitor returning `yes`, or a scorer crossing a threshold) can surface as actionable [Signals](/docs/signals) – so a human gets notified about important sessions without anyone manually reviewing replays.
