...
Go to **Replay Vision** in the PostHog app and click **New scanner**. The wizard walks you through three steps:

### 1. Choose a template

Pick a pre-configured template to get started quickly, or select **Create from scratch** for a fully custom scanner. Templates cover common use cases like dead-end detection, session summary, user intent, frustration score, and session outcome. Each template pre-fills the scanner type, prompt, and configuration.

### 2. Configure

Give your scanner a name and write a prompt describing what to look for. Be concrete and avoid speculation – for example, *"Return true if someone appears stuck on a page – scrolling without engaging, hovering over elements with no clear CTA, or abandoning shortly after arriving."*

Configure the type-specific options based on your [scanner type](#scanner-types):
...
Set **recording filters** to choose which sessions the scanner applies to (by person properties, session properties, cohorts, or events) and a **sampling rate** to control how many matching sessions get scanned.

Optionally enable [Signals](#signals) so notable findings can trigger alerts.

Click **Save** to create the scanner. It's enabled by default.
