...
We've built PostHog as a multitenant system on a Clickhouse cluster that we host. This is great for most of our analytical queries that we make because we expect that most of the queries being made are generated by a user at a browser and because of that we can expect that we will return data relatively quickly (on the order of minutes). Data warehouses have entirely different expectations. It’s not atypical for a modeling query in a warehouse to run for hours or even days. This is just not something we can do on our multi-tenant CH cluster. We restart nodes. Nodes restart themselves. We optimize for querying relatively hot data. The prospect of an unbounded number of queries hitting our very not elastic CH cluster is basically untenable. This would eventually cause problems across our entire application.


### Challenge 2: ClickHouse as a query engine

We use Clickhouse internally for the parts of our product where it really shines: fast, scalable analytics over large event datasets, with schemas and queries designed specifically around its strengths. All of our analytics data is stored in ClickHouse and that use case isn’t changing; the challenge our users are facing is using ClickHouse as a general-purpose warehouse, where they need lots of sources connected, modelling flexibility, schema changes, and mutations.
...
- **S3/Deltalake/Iceberg support took a long time to mature.** We wanted to leverage Clickhouse's query engine to query data catalog's however our success was limited and we mostly had to rely on raw Parquet without the benefits of catalogued data.
...
As companies mature, their data operations and use cases often become more sophisticated. We experienced this first hand at PostHog while building out our revenue reporting. What started as a handful of queries and dashboards quickly became a maze of logic and context shared across multiple people. While we instinctively wanted to reach for dbt to model our data and adhere to best practices in managing our queries, we simply could not expose direct connections to the Clickhouse cluster without deepening the challenges of multitenancy. On top of that, our query language, HogQL is not a language that's already widely supported in the ecosystem. Despite being able to conveniently centralize our business data alongside PostHog event data, the lack of data tooling in our v1 of data warehousing left us and many more sophisticated data teams without long term confidence.
